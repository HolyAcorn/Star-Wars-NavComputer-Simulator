...
namespace Sisus.Shared.EditorOnly
{
	public static class InspectorContents
	{
		...
		internal static IEnumerable<(Editor, IMGUIContainer)> GetComponentHeaderElementsFromInspector(Editor gameObjectEditor)
		{
		...
		private static EditorWindow GetGameObjectEditorInspector(Editor gameObjectEditor)
		{
			foreach(var inspector in allInspectors)
			{
		...
		private static EditorWindow GetPropertyEditorWindow(Editor componentEditor)
		{
		...
		private static (Editor, IMGUIContainer)? GetFirstComponentHeaderElement(VisualElement parentElement)
		{
			if(parentElement is null)
			{
				return null;
